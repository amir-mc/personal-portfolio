...
export const BentoGridItem = ({
  ...
  const handleCopy = async () => {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      try {
        await navigator.clipboard.writeText('https://github.com/amir-mc');
        setCopied(true);
      } catch (err) {
      ...
      textArea.value = 'https://github.com/amir-mc';
    ...
    >
      <div className={`${id === 6 && 'flex justify-center'} h-full`}>
        ...
        <div className={cn(titleClassName, 'group-hover/bento:translate-x-2 transition duration-200 relative md:h-full min-h-40 flex flex-col px-5 p-5 lg:p-10')}>
          <div className="font-sans font-extralight text-sm md:text-xs dark:text-neutral-300">
            {description} 
          </div>
            ...
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-8">
                {leftLists.map((item, i) => (
                  <span key={i} className="lg:py-4 lg:px-3 py-2 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center text-white-100 bg-[#10132E]">
                    {item}
                  </span>
                ))}
              ...
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-8">
                ...
                {rightLists.map((item, i) => (
                  <span key={i} className="lg:py-4 lg:px-3 py-2 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center text-white-200 bg-[#10132E]">
                    {item}
                  </span>
                ))}
              </div>
